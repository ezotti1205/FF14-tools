/* universalis.js — Universalis API v2 クライアント（共通）
 *
 * GET https://universalis.app/api/v2/{world|dc}/{itemIds}
 *   - itemIds はカンマ区切りでまとめて投げる
 *   - 結果は呼び出し側の Store に15分キャッシュ（store.getFreshPrice / putPrice）
 *
 * 「作る vs 買う」と「市場」の2タブが使うため、Store を差し込む形の
 * ファクトリにしてあります。localStorage のキー体系はタブごとに別のままです
 * （craft は ff14cp.* / watch は ff14watch.v1.*）。
 *
 *   var U = FF14.core.createUniversalis({ store: FF14.craft.Store, appName: 'ff14-craft-profit' });
 *
 * 識別情報について:
...
(function () {
  ...
  var APP_VER = '1.0';
  var CHUNK = 20;                // 1リクエストの件数。40件以上は必ず504になるため20に抑えている
  var TTL = 15 * 60 * 1000;      // 15分
  var LISTINGS = 20;             // 取得する出品件数
  var ENTRIES = 20;              // 取得する取引履歴件数
  var TIMEOUT = 20000;
  var RETRIES = 2;               // 同じ要求でも504が散発的に返るので取り直す
  var RETRY_WAIT = 900;

  function ident(appName) {
    return 'appName=' + encodeURIComponent(appName + '/' + APP_VER) +
           '&contact=' + encodeURIComponent(appName + ' (single-user browser tool)');
  }
  ...
  /** 504 / 一時的なネットワークエラーを段階的に待って取り直す */
  function fetchRetry(url, left) {
    return fetchWithTimeout(url).catch(function (err) {
      if (left <= 0) throw err;
      return delay(RETRY_WAIT * (RETRIES - left + 1)).then(function () {
        return fetchRetry(url, left - 1);
      });
    });
  }
  ...
  /**
   * クライアントを1つ作る。呼び出し側ごとに失敗状態を独立して持つ。
   * @param {{store:object, appName:string}} cfg
   *   store は getFreshPrice / putPrice / getWorlds / putWorlds を実装していること
   */
  function createUniversalis(cfg) {
    var Store = cfg.store;
    var IDENT = ident(cfg.appName || 'ff14-tools');
    ...
    function ensurePrices(scope, itemIds, opts) {
      ...
      return chunks.reduce(function (p, ids) {
        return p.then(function () {
          ...
          return fetchRetry(url, RETRIES).then(function (json) {
    ...
    return {
      ...
      errorFor: function (itemId) { return failed[itemId] || null; },
      hasFailures: function () { return Object.keys(failed).length > 0; },
      clearFailures: function () { failed = {}; },
      loadWorlds: loadWorlds,
      ttlMs: function () { return TTL; }
    };
  }

  FF14.core.createUniversalis = createUniversalis;
})();
