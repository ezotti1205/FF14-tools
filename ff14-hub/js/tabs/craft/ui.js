/* ui.js — 元 ff14-craft-profit/js/ui.js。中身は無変更で、グローバルを汚さないよう
   IIFEで包んで FF14.craft.UI として公開しているだけです。 */
(function () {
  var GameData = FF14.craft.GameData;
  var Icons = FF14.core.Icons;   // アイテムアイコン（XIVAPI v2）
/* ui.js — 描画まわり */
var UI = (function () {
  'use strict';
  ...
  function renderTarget(el, result) {
    ...
    var html = '';
    html += Icons.placeholder(id, GameData.nameOf(id), 'lg');
  ...
  function renderTree(el, result, state) {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      html += Icons.placeholder(row.itemId, GameData.nameOf(row.itemId));
  ...
  function renderShop(el, result) {
    ...
    for (var i = 0; i < result.purchase.length; i++) {
      ...
      html += '<div class="kv"><span class="k">' +
              Icons.placeholder(it.itemId, GameData.nameOf(it.itemId), 'sm') + ' ' +
              esc(GameData.nameOf(it.itemId)) +
