...
css/motion.css            操作したときの動き。各タブのCSSより後に読み込みます
...
## 操作したときの動き（`css/motion.css`）

タブを切り替えた・候補リストが開いた・検索が走った、といった操作に短い動きを付けています。
時間の指定は `--mo-fast` / `--mo-base` / `--mo-slow` / `--mo-step` の4つのトークンに集約してあるので、
**速さを変えたいときは `css/motion.css` の `:root` だけ**を書き換えてください。

| 操作 | 動き |
| --- | --- |
| タブ切り替え | パネルが下から6pxフェードイン（`mo-rise`） |
| 候補リスト（3タブ共通の `.suggest`） | 上から4px落ちて開く（`mo-pop`）。`hidden` が外れた瞬間に再生されるのでJS不要 |
| 検索・価格取得・DB読み込み | 入力欄の下を細いバーが往復（`.is-busy` → `mo-sweep`） |
| 一覧の描画 | 行が下から4pxフェードイン（`mo-row`） |
| ボタン | 押した瞬間に `scale(.97)`。ホバーは色だけ |
| ノードの出現状態 | 「出現中」「まもなく」に変わったときの背景色がなめらかに変化 |

重くしないための決めごと:

- **動かすのは `transform` / `opacity` / 色だけ。** `width` / `height` / `top` を動かさないので、
  レイアウトの再計算が走りません。
- **一覧の登場アニメは先頭12件まで。** 13件目以降は遅延なしで一斉に出ます。
  40件でも225件でも、同時に走るアニメは最大12件です。
- **「ノード」タブの行には登場アニメを付けていません。** `tick()` が1秒ごとに
  全225行を `appendChild` で並べ替えるため、付けると毎秒225件が再生されてしまいます。
  代わりに状態が変わったときの色変化（`transition`）だけにしてあります。
  `transition` は値が変わったときにしか発火しないので、並べ替えでは動きません。
- **OSの「視差効果を減らす」（`prefers-reduced-motion`）を有効にしている場合は全部止まります。**
  上記4トークンをほぼ0に上書きするだけで全体が止まる作りです。

実測（225行を並べ替えて強制レイアウトまで走らせた場合の中央値）:
`motion.css` 有り 20.7ms / 無し 18.9ms → **差は約1.8ms**。並べ替え直後に増える
アニメーションは0件です。
